<!-- src/components/liquidation/LiquidationAttachmentsModal.vue -->
<script setup>
...
/* Helpers (same as utilization, re-used) */
...
const onFileChange = async (e) => {
    ...
    if (!isAllowedFile(file)) {
        await Swal.fire('Unsupported file', 'Only images (PNG/JPEG/WEBP), PDF, DOCX, and XLSX are allowed.', 'warning')
        return
    }
    try {
...
const openPreview = (att) => {
    if (att.data && String(att.data).startsWith('data:') && isImageMime(att.mime)) {
        ...
        if (win) {
            win.document.write(`<img src="${att.data}" style="display:block;max-width:100%;height:auto;margin:0 auto;object-fit:contain"/>`)
        }
        return
    }
        ...
        <div class="bg-white p-4 rounded-xl shadow-xl w-[980px] max-h-screen overflow-auto">
            <div class="flex items-center justify-between mb-3">
                <h2 class="text-lg font-semibold">
                    Liquidation Attachments — <span class="text-gray-600">{{ current?.reference_code || '' }}</span>
                </h2>
