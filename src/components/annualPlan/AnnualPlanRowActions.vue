...
const reasonFor = (key) => {
  if (isBusy.value) return 'Action is temporarily unavailable.'
  switch (key) {
    case 'view':
      return 'Drafts open in edit mode.'
    case 'submit':
      return 'Only draft annual plans can be submitted.'
    case 'edit':
      return 'Only draft annual plans can be edited.'
    case 'approve':
      return 'Only moderators can approve pending plans.'
    case 'reject':
      return 'Only moderators can reject pending plans.'
    case 'cancel':
      return 'Only moderators can cancel pending or approved plans.'
    case 'delete':
      return 'Only moderators can delete plans.'
    default:
...
const primaryAction = computed(() => {
  if (isSplitWorkflow.value) {
    return { key: 'approve', label: 'Approve', icon: mdiCheckDecagram, emit: 'approve', allowed: canModerate.value && !isBusy.value, reason: reasonFor('approve') }
  }
  if (canSubmit.value) {
    return { key: 'edit', label: 'Edit', icon: mdiPencil, emit: 'edit', allowed: canEdit.value && !isBusy.value, reason: reasonFor('edit') }
  }
  if (canView.value) {
    return { key: 'view', label: 'View', icon: mdiEye, emit: 'view', allowed: canView.value && !isBusy.value, reason: reasonFor('view') }
  }
...
const moreItems = computed(() => {
  const items = [
    { key: 'view', label: 'View', icon: mdiEye, emit: 'view', allowed: canView.value && !isBusy.value, reason: reasonFor('view') },
    { key: 'edit', label: 'Edit', icon: mdiPencil, emit: 'edit', allowed: canEdit.value && !isBusy.value, reason: reasonFor('edit') },
    { key: 'submit', label: 'Submit', icon: mdiSend, emit: 'submit', allowed: canSubmit.value && !isBusy.value, reason: reasonFor('submit') },
    { key: 'approve', label: 'Approve', icon: mdiCheckDecagram, emit: 'approve', allowed: canModerate.value && !isBusy.value, reason: reasonFor('approve') },
    { key: 'reject', label: 'Reject', icon: mdiCloseOctagon, emit: 'reject', allowed: canModerate.value && !isBusy.value, reason: reasonFor('reject'), tone: 'danger' },
    { key: 'cancel', label: 'Cancel', icon: mdiCancel, emit: 'cancel', allowed: canCancel.value && !isBusy.value, reason: reasonFor('cancel'), tone: 'warning' },
    { key: 'delete', label: 'Delete', icon: mdiTrashCan, emit: 'delete', allowed: canDelete.value && !isBusy.value, reason: reasonFor('delete'), tone: 'danger' },
  ]
...
const positionMenu = (el, styleRef, width, heightGuess = 220) => {
...
const toggleMenu = () => {
  ...
  if (open.value) positionMenu(btnRef.value, menuStyle, 208, 220)
  ...
  <div class="relative">
    <div class="flex items-center gap-1.5 md:gap-2">
      ...
      <div class="relative inline-flex">
        <IconifyButton :icon-path="mdiMessageTextOutline" color="text-slate-700" label="" tooltip="Remarks"
          size="sm" :disabled="busy" @click="$emit('remarks', row)" />
      ...
      <template v-if="isSplitWorkflow">
        <div class="relative inline-flex items-center" ref="workflowBtnRef">
          <button
            ...
            :disabled="busy" @click="!busy && $emit('approve', row)">
          ...
          <button
            ...
            :aria-expanded="workflowOpen" aria-haspopup="menu" title="More workflow" :disabled="busy"
            @click="!busy && toggleWorkflowMenu()">
