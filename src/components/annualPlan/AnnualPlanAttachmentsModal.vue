<!-- src/components/annualPlan/AnnualPlanAttachmentsModal.vue -->
<script setup>
import { ref, computed, watch } from 'vue'
import Swal from 'sweetalert2'
import { useAnnualPlanStore } from '@/stores/annualPlan'
import {
    mdiFilePdfBox, mdiFileWordBox, mdiFileExcelBox, mdiFileImage, mdiFileDocumentOutline,
    mdiOpenInNew, mdiTrashCan, mdiUpload, mdiDownload
} from '@mdi/js'

const props = defineProps({ modelValue: { type: Boolean, default: false }, row: { type: Object, default: null } })
...
const isAllowedFile = (file) => (file?.type && ALLOWED_MIMES.has(file.type)) || (file?.name && ALLOWED_EXTS.has(extFromName(file.name)))
...
const attachments = computed(() =>
    (rawAttachments.value || []).map((a, idx) => {
        const mime = a.mime || mimeFromDataUrl(a.data) || ''
        const ext = extFromName(a.name || '') || extForMime(mime)
        const size = a.size ?? null
        const name = a.name || `${labelForMime(mime)} ${idx + 1}`
        return { id: a.id ?? idx + 1, name, data: a.data || '', url: a.url || a.href || a.path || '', mime, ext, size, filename: ensureFilenameWithExt(name, ext) }
    })
)
...
const onFileChange = async (e) => {
    ...
    if (file.size > MAX_SIZE) { await Swal.fire('File too large', 'Maximum size is 10 MB.', 'warning'); return }
    if (!isAllowedFile(file)) { await Swal.fire('Unsupported file', 'Only images, PDF, DOCX, XLSX allowed.', 'warning'); return }
    try {
...
const removeAttachment = async (attId) => {
    ...
    const ok = await Swal.fire({ title: 'Remove attachment?', icon: 'warning', showCancelButton: true, confirmButtonText: 'Remove', confirmButtonColor: '#d33' })
...
const openPreview = (att) => {
    if (att.data && String(att.data).startsWith('data:') && isImageMime(att.mime)) {
        ...
        if (win) win.document.write(`<img src="${att.data}" style="display:block;max-width:100%;height:auto;margin:0 auto;object-fit:contain"/>`)
        return
    }
...
const downloadAttachment = async (att) => {
    if (att.data && String(att.data).startsWith('data:')) { triggerDownload(dataURLToBlob(att.data), att.filename); return }
    if (att.url) {
        try {
            ...
            const a = document.createElement('a'); a.href = String(att.url); a.setAttribute('download', att.filename); document.body.appendChild(a); a.click(); a.remove()
        }
    }
}
        ...
        <div class="bg-white p-4 rounded-xl shadow-xl w-[980px] max-h-screen overflow-auto">
            <div class="flex items-center justify-between mb-3">
                <h2 class="text-lg font-semibold">
                    Annual Plan Attachments — <span class="text-gray-600">{{ current?.reference_code || '' }}</span>
                </h2>
            ...
            <div class="flex items-center justify-between gap-2">
                <div class="text-sm text-gray-600">
                    School Year: <strong>{{ current?.school_year || '—' }}</strong>
                </div>
            ...
            <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                <div v-for="a in attachments" :key="a.id" class="border rounded-lg p-2 bg-gray-50 flex flex-col">
                    <div class="aspect-video bg-white border rounded overflow-hidden flex items-center justify-center">
                        <img v-if="a.data && a.data.startsWith('data:') && a.mime?.startsWith('image/')" :src="a.data"
                            class="object-contain w-full h-full" alt="Attachment preview" />
                        <div v-else class="flex items-center justify-center w-full h-full">
                            <svg style="width:64px;height:64px" viewBox="0 0 24 24" :class="{
                                'text-red-600': a.mime === 'application/pdf',
                                'text-blue-600': a.mime === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
                                'text-emerald-600': a.mime === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                                'text-gray-500': !a.mime || (!a.mime.startsWith('image/') && a.mime !== 'application/pdf'),
                            }">
