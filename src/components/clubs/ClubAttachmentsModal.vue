...
watch(
    ...
    async (v) => {
        if (v && props.row?.id) {
            ...
            current.value = store.selectedClub || props.row
        }
    },
    { immediate: false }
)
...
const onFileChange = async (e) => {
    ...
    try {
        ...
        current.value = store.selectedClub
    } finally {
        ...
        <div class="bg-white p-4 rounded-xl shadow-xl w-[980px] max-h-screen overflow-auto">
            <div class="flex items-center justify-between mb-3">
                <h2 class="text-lg font-semibold">
                    Attachments — <span class="text-gray-600">{{ current?.name || '' }}</span>
                </h2>
            ...
            <div class="flex items-center justify-between gap-2">
                <div class="text-sm text-gray-600">
                    Ref: <strong>{{ current?.code || '—' }}</strong>
                </div>
