...
async function fetchItem(idOrRef) {
  ...
  try {
    ...
    if (isNumericId(idOrRef)) {
      item.value = await activityDesignService.getById(idOrRef)
    } else {
      const res = await activityDesignService.list({ q: idOrRef, limit: 1, page: 1 })
      item.value = Array.isArray(res?.data) && res.data.length ? res.data[0] : null
    }
    if (!item.value) error.value = 'No matching Activity Design found'
  } catch (e) {
...
<template>
  <div class="min-h-screen bg-gray-50 flex items-center justify-center p-4">
    <div class="w-full max-w-2xl bg-white rounded-2xl shadow border p-5">
      <div class="mb-4">
        <h1 class="text-lg font-semibold">Verify: Activity Design</h1>
      ...
      <div v-else class="space-y-3">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
          ...
          <div class="md:col-span-2">
            <div class="text-gray-500">Activity Name</div>
            <div class="font-medium">{{ item.name_of_activity || '—' }}</div>
          </div>
          <div>
            <div class="text-gray-500">Club/Department</div>
            <div class="font-medium">{{ item.office_department || item?.club?.name || '—' }}</div>
          </div>
          <div>
